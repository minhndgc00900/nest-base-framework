...
const nestLikeConsoleFormat = () =>
  winston.format.printf(({ context, level, timestamp, message, ...meta }) => {
    ...
    return (
      ...
      ('undefined' !== typeof context ? `${chalk.default.yellow('[' + context + ']')} ` : '') +
      `${messageString}` +
...
class AppLogger implements AppLoggerService {
  constructor(private context: string, private readonly logger: winston.Logger) {}
...
export class LoggerFactory {
  ...
  static create(context: string | Function): AppLoggerService {
    // tslint:disable-next-line: ban-types
    const contextName = typeof context === 'string' ? context : (context as Function).name;
